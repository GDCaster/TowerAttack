...
function updateGame(room, logMsg = null) {
    ...
    io.to(room.id).emit('updateGame', {
        players: room.players.map(p => ({
            id: p.id, name: p.name, coins: p.coins, cardsCount: p.cards.length,
            isAlive: p.isAlive, lostCards: p.lostCards || [],
            cards: p.isAlive ? p.cards : [], color: p.color
        })),
...
function loseCard(room, playerId, reason) {
    ...
    // แจ้งเตือนไปที่ PlayerId นั้นๆ ว่าต้องทิ้งการ์ด
    if (p.cards.length > 0) {
        io.to(playerId).emit('forceLoseCard', { message: `คุณ${reason} กรุณาเลือกการ์ดทิ้ง 1 ใบ` });
    } else {
        p.isAlive = false;
...
function processAction(room, playerId, actionType, targetId) {
    ...
    if (actionType === 'Income') { 
        player.coins++; updateGame(room, `${player.name} หยิบ 1 เหรียญ`); nextTurn(room); return;
    }
    if (actionType === 'Coup') { 
        if (player.coins < 7) return;
        player.coins -= 7;
        const target = room.players.find(p => p.id === targetId);
        updateGame(room, `${player.name} รัฐประหารใส่ ${target.name}!`);
        
        // Logic Coup: เป้าหมาย (Target) เป็นคนเลือกทิ้งการ์ดเอง
    ...
    room.currentAction = { type: actionType, sourceId: playerId, targetId: targetId, status: 'pending', cost: cost };
    const thaiNames = { 'Foreign Aid': 'ขอเงินช่วยเหลือ (2)', 'Tax': 'เก็บภาษี (3)', 'Steal': 'ขโมยเหรียญ', 'Assassinate': 'ลอบสังหาร', 'Exchange': 'เปลี่ยนการ์ด' };

    io.to(room.id).emit('actionBroadcast', {
        action: actionType, actionNameTH: thaiNames[actionType],
        sourceName: player.name, sourceId: player.id, targetId: targetId
    });

    // Auto Resolve เมื่อเวลาหมด (8 วินาที)
...
function resolveAction(room) {
    ...
    if (act.type === 'Foreign Aid') { source.coins += 2; msg = `${source.name} ได้รับเงินช่วยเหลือ (+2)`; }
    else if (act.type === 'Tax') { source.coins += 3; msg = `${source.name} เก็บภาษี (+3)`; }
    else if (act.type === 'Steal') { const stolen = Math.min(target.coins, 2); target.coins -= stolen; source.coins += stolen; msg = `${source.name} ขโมย ${stolen} เหรียญจาก ${target.name}`; }
...
io.on('connection', (socket) => {
    ...
    socket.on('create_room', ({ name, customId }) => {
        ...
        if (customId && customId.trim() !== "") {
            ...
            if (rooms[roomId]) return socket.emit('error_msg', 'ชื่อห้องซ้ำ (มีคนใช้แล้ว)');
        } else {
            do { roomId = Math.random().toString(36).substr(2, 5).toUpperCase(); } while (rooms[roomId]);
        }
    ...
    socket.on('join_room', ({ roomId, name }) => {
        ...
        if(room.players.length >= 6) return socket.emit('error_msg', 'ห้องเต็ม');
    ...
    function joinRoomLogic(socket, roomId, username, isHost) {
        ...
        room.players.push({ id: socket.id, name: username, coins: 0, cards: [], lostCards: [], isAlive: true, isHost, isReady: false, color });
    ...
    socket.on('select_color', ({ roomId, color }) => {
        const room = rooms[roomId];
        if(!room) return;
        if(!room.players.some(p => p.color === color && p.id !== socket.id)) {
    ...
    socket.on('start_game_request', (roomId) => {
        const room = rooms[roomId];
        if(!room) return;
        if(room.players.length < 2) return socket.emit('error_msg', 'คนไม่พอ (Min 2)');
        if(!room.players.every(p => p.isReady)) return socket.emit('error_msg', 'รอเพื่อนกดพร้อม');
        
        room.status = 'playing';
        room.deck = createDeck();
        room.players.forEach(p => {
            p.cards = [room.deck.pop(), room.deck.pop()];
            p.coins = 0; // [FIXED] Start with 0 Coins
            p.isAlive = true; p.lostCards = [];
        });
    ...
    function updateLobby(roomId) {
        if(rooms[roomId]) io.to(roomId).emit('update_lobby', { players: rooms[roomId].players, colors: COLORS });
    }
    ...
    socket.on('react', ({ roomId, type }) => {
        ...
        if (type === 'challenge') {
            ...
            io.to(actor.id).emit('requestProof', { action: room.currentAction.type, message: `${reactor.name} ท้าจับโกหกคุณ!` });
            ...
            io.to(room.id).emit('blockBroadcast', { blockerName: reactor.name, message: `${reactor.name} ทำการ Block!` });
            room.actionTimer = setTimeout(() => {
             ...
             io.to(blocker.id).emit('requestProof', { action: 'Block', message: `${reactor.name} สงสัยการ Block ของคุณ!` });
    ...
    socket.on('provideProof', ({ roomId, cardName }) => {
        ...
        if (isCorrect) {
            ...
            room.deck.push(cardName); room.deck.sort(() => Math.random() - 0.5);
            actor.cards.push(room.deck.shift());
            if (room.currentAction.status === 'challenging') resolveAction(room); 
            else { updateGame(room, "Block สำเร็จ (มีการ์ดจริง)"); nextTurn(room); }
        } else {
            loseCard(room, actor.id, "โดนจับโกหกได้");
            if (room.currentAction.status === 'challenging') {
                if(actType === 'Assassinate') { const src = room.players.find(p => p.id === room.currentAction.sourceId); src.coins += 3; }
                updateGame(room, "การกระทำล้มเหลว (โกหก)"); nextTurn(room);
            } else { updateGame(room, "Block ล้มเหลว (โกหก)"); resolveAction(room); }
        }
    });
    ...
    socket.on('discardCard', ({ roomId, cardName }) => {
        ...
        if(idx > -1) {
            p.cards.splice(idx, 1); p.lostCards.push(cardName);
            
            // [FIXED] Secret Exchange: ถ้าเป็นการ Exchange จะไม่บอกชื่อการ์ด
            if (room.currentAction && room.currentAction.type === 'Exchange' && room.currentAction.sourceId === p.id) {
                updateGame(room, `${p.name} เก็บการ์ดเข้ากอง`);
            } else {
                updateGame(room, `${p.name} ทิ้งการ์ด ${cardName}`);
            }

            if(p.cards.length === 0) { p.isAlive = false; updateGame(room, `${p.name} ถูกกำจัด!`); }
            
            // ถ้าเป็นการตายจาก Coup หรือ Assassin หรือ Challenge ให้เปลี่ยนเทิร์น
            // แต่ถ้า Exchange ต้องรอทิ้งครบ 2 ใบ (Logic อยู่ที่ Client ส่ง finishExchange)
            const alive = room.players.filter(pr => pr.isAlive);
            if (alive.length > 1 && (!room.currentAction || room.currentAction.type !== 'Exchange')) nextTurn(room);
        }
    });
    
    socket.on('finishExchange', ({roomId}) => { const room = rooms[roomId]; if(room) nextTurn(room); });

    socket.on('sendChat', ({ roomId, msg }) => {
        const room = rooms[roomId];
        if(room) { const p = room.players.find(player=>player.id===socket.id); io.to(roomId).emit('chatMessage', { name: p.name, msg, color: p.color }); }
    });
    
    socket.on('disconnect', () => {
         for (const rid in rooms) {
             const r = rooms[rid]; const idx = r.players.findIndex(p => p.id === socket.id);
             if(idx !== -1) { r.players.splice(idx, 1); if(r.players.length === 0) delete rooms[rid]; else updateLobby(rid); }
         }
    });
});
