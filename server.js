            ...
            if (u.aiming) {
                ...
                if (u.aimTargetType === 'base') {
                     if (room.bases[u.side === 'left' ? 'right' : 'left'] > 0) {
                         targetExists = true; tX = enemyBaseX; tY = WORLD_H/2;
                     }
                } else {
                ...
                if (now - u.aimStartTime >= SPECS.sniper.aimTime) {
                    ...
                    if (u.aimTargetType === 'base') {
                        ...
                        room.effects.push({ type: 'dmg', x: tX, y: tY, val: u.dmg });
                        if (room.bases[targetSide] <= 0) endGame(room, u.side);
                    } else {
                        const targetUnit = room.units.find(e => e.id === u.aimTargetId);
                        if (targetUnit) {
                            targetUnit.hp -= u.dmg;
                            room.effects.push({ type: 'dmg', x: targetUnit.x, y: targetUnit.y, val: u.dmg });
...
function spawnProjectile(room, owner, target) {
    room.projectiles.push({
        ...
        speed: owner.type === 'cannon' ? 14 : 10, dmg: SPECS[owner.type].dmg, 
        radius: SPECS[owner.type].radius || 10, type: owner.type, side: owner.side
    });
}
